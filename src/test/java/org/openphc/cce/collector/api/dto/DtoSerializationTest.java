...
class DtoSerializationTest {
    ...
    // ════════════════════════════════════════════════════════════════
    // RejectedEventDto — Serialization
    // ════════════════════════════════════════════════════════════════

    @Nested
    @DisplayName("RejectedEventDto — Serialization")
    class RejectedEventDtoSerialization {

        @Test
        @DisplayName("serializes expected field names")
        void fieldNames() throws Exception {
            RejectedEventDto dto = RejectedEventDto.builder()
                    .id("uuid-456")
                    .cloudEventsId("evt-002")
                    .source("rhie-mediator")
                    .eventType("org.openphc.cce.encounter")
                    .rejectionReason("INVALID_FHIR")
                    .errorDetails("Missing resourceType")
                    .receivedAt(OffsetDateTime.now(ZoneOffset.UTC))
                    .build();
            ...
            assertThat(node.has("errorDetails")).isTrue();
            assertThat(node.has("receivedAt")).isTrue();
        }
        ...
        void nullOmitted() throws Exception {
            ...
            assertThat(node.has("rejectionReason")).isTrue();
            assertThat(node.has("cloudEventsId")).isFalse();
            assertThat(node.has("errorDetails")).isFalse();
        }
    }
}
